...
const ProductCard = ({ title, seller, salePrice, price, logo, img, id, addToCart, addToFav }) => {
  const { addToFavorites, removeFromFav } = useAppContext();

  const handleAddToCart = () => {
    addToCart({ id, title, price, salePrice, img, logo, seller, quantity: 1 });
  };
  const handleAddToFavorites = () => {
    addToFavorites({ id, title, price, salePrice, img });
  };
  return (
      ...
      <div className={styles.info}>
        ...
        <div className={styles.buttons}>
          <button className={styles.like}>
            <img src="/favorite.png" alt="favorite" onClick={ handleAddToFavorites} />
            <div>Sevimlilar</div>
          </button>
...
export const InFav = ({ img, price, salePrice, title, id }) => {
  ...
  return (
    <div className={styles.inCart}>
      ...
      <div>
        <img src="/addToCaееtpng" alt="tobag" />
        <img src="/favorite.png" onClick={handleAddToFavorites} alt="fav" />
      </div>
    </div>
  );
};
