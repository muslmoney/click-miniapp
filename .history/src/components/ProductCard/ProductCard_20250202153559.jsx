...
const ProductCard = ({ title, seller, salePrice, price, logo, img, id, addToCart }) => {
  const { favorites, addToFavorites, removeFromFav } = useAppContext();

  // Проверка, находится ли продукт в избранном
  const isFavorite = favorites.some(product => product.id === id);

  // Функция для добавления или удаления из избранных
  const handleFavoriteToggle = () => {
    if (isFavorite) {
      removeFromFav(id);  // Удаляем из избранных
    } else {
      addToFavorites({ id, title, price, salePrice, img });  // Добавляем в избранное
    }
  };
      ...
      <div className={styles.info}>
        ...
        <div className={styles.buttons}>
          <button className={styles.like} onClick={handleFavoriteToggle}>
            {/* Переключение иконки в зависимости от того, находится ли продукт в избранном */}
            <img
              src={isFavorite ? "/favorite-filled.png" : "/favorite.png"} 
              alt="favorite"
            />
            <div>{isFavorite ? "Избранное" : "Добавить в избранное"}</div>
          </button>
  ...
  return (
    <div className={styles.inCart}>
      ...
      <div>
        ...
        <img
          src="/favorite-filled.png"
          alt="remove from favorites"
          onClick={() => removeFromFavorites(id)} // Удалить из избранного
        />
      </div>
    </div>
  );
